...
- Claude Code 플러그인으로 제공되며 background monitor 포함
...
- `monitors/monitors.json` - Claude Code 안에서 프록시 자동 시작
...
CLI 플래그는 환경 변수보다 우선합니다. 내장 monitor는
...
설치된 플러그인을 일반 `claude`로 실행하면 monitor는 프록시를 시작할 수 있지만,
